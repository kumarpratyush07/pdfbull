...
import { Download, RefreshCw, FileText, Lock, Unlock, Key, AlertCircle, Eye, Image, ArrowRight } from 'lucide-react';
import { formatBytes } from '../lib/utils';
import PdfPreviewModal from './PdfPreviewModal';

interface PdfToImageDashboardProps {
  file: UploadedFile;
  onRemoveFile: () => void;
  onConvert: (format: 'jpg' | 'png') => void;
  onClear: () => void;
...
const PdfToImageDashboard: React.FC<PdfToImageDashboardProps> = ({ 
  file, 
  onRemoveFile,
  onConvert,
  onClear,
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden text-center p-12 transition-colors duration-300">
           ...
           <div className="flex flex-col sm:flex-row gap-4 justify-center">
             ...
             >
               ...
               {isZip ? 'Download ZIP' : 'Download Image'}
             </a>
             <button 
               onClick={onClear}
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 w-full">
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden transition-colors duration-300">
          <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
            <div>
               <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">PDF to Image</h2>
               <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                 Convert document pages to high-quality images
               </p>
            </div>
            <button 
                  ...
                  <div className="max-w-full">
                     ...
                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-lg">
                       <div 
                         ...
                         `}
                       >
                         <div className={`p-3 rounded-lg ${format === 'jpg' ? 'bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}>
                           <Image className="w-6 h-6" />
                         </div>
                         <div>
                           <div className="font-bold text-slate-900 dark:text-slate-100">JPG</div>
                           <div className="text-xs text-slate-500 dark:text-slate-400">Best for photos, smaller size</div>
                         </div>
                       </div>
                       ...
                       <div 
                         ...
                         `}
                       >
                         <div className={`p-3 rounded-lg ${format === 'png' ? 'bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}>
                           <Image className="w-6 h-6" />
                         </div>
                         <div>
                           <div className="font-bold text-slate-900 dark:text-slate-100">PNG</div>
                           <div className="text-xs text-slate-500 dark:text-slate-400">Best for text, lossless quality</div>
                         </div>
                       </div>
                     </div>
                  </div>
            ...
            {isProcessing ? (
              <div className="flex-1 flex flex-col justify-center max-w-sm ml-auto">
                ...
                <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2 overflow-hidden">
                  <div 
                    className="bg-purple-600 dark:bg-purple-500 h-full rounded-full transition-all duration-300 ease-out"
                ...
                <button 
                  ...
                  className={`
                    ...
                    ${(needsPassword || isProcessing)
                      ...
                      : 'bg-purple-600 hover:bg-purple-700 hover:shadow-purple-200 dark:hover:shadow-purple-900/30'
                    }
                  `}
                >
                  Convert to Images
...
export default PdfToImageDashboard;